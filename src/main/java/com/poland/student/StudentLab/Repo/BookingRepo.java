...
public interface BookingRepo extends JpaRepository<Booking, Integer> {
    ...
    boolean existsByRoomAndDate(@Param("room_id") int roomId, @Param("date") LocalDateTime date);

    @Query("SELECT b FROM Booking b WHERE b.room.id = :room_id AND b.date = :date")
    Booking findByRoomAndDate(@Param("room_id") int roomId, @Param("date") LocalDateTime date);
}
